...
script_name = "surv_public_msos_v3.1.R"
...
if (pamst != "") {
	...
	pamst_clin = sub_clin[sub_clin[,"Pam50Subtype"] == pamst,]
	pamst_clin = pamst_clin[complete.cases(pamst_clin$pid),]
	cat("\tFiltered patient number: ", dim(pamst_clin)[1], "\n")
} 
	if (hrtype != "") {
		if (length(hrtype) != 3) {stop("Not enough info for hormone receptor status!!!")}
		cat("\tER: ", hrtype[1], "\n")
		if (hrtype[1] == "P") {
			hr_clin = sub_clin[sub_clin$ER.Expr == "+",]
			hr_clin = hr_clin[complete.cases(hr_clin$pid),]
		}
		if (hrtype[1] == "N") {
			hr_clin = sub_clin[sub_clin$ER.Expr == "-",]
			hr_clin = hr_clin[complete.cases(hr_clin$pid),]
		}
		cat("\t\tFiltered patient number: ", dim(hr_clin)[1], "\n")

		cat("\tPR: ", hrtype[2], "\n")
		if (hrtype[2] == "P") {
			hr_clin = hr_clin[hr_clin$PR.Expr == "+",]
			hr_clin = hr_clin[complete.cases(hr_clin$pid),]
		}
		if (hrtype[2] == "N") {
			hr_clin = hr_clin[hr_clin$PR.Expr == "-",]
		...
		cat("\tHer2: ", hrtype[3], "\n")
		if (hrtype[3] == "P") {
			hr_clin = hr_clin[hr_clin$Her2.Expr == "+",]
			hr_clin = hr_clin[complete.cases(hr_clin$pid),]
		}
		if (hrtype[3] == "N") {
			hr_clin = hr_clin[hr_clin$Her2.Expr == "-",]
...
sub_clin<-unique(rbind(pamst_clin, hr_clin))
