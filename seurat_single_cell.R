...
sr_QC = function (sc_obj, feaRNA_min, feaRNA_max, perMT, resdir, proFlag) {
	...
	sc_obj <- subset(sc_obj, subset = nFeature_RNA > 200 & nFeature_RNA < 2500 & percent.mt < 10) ###check feature plot
	...
	ggsave(plot = plot2, filename = paste(res_dir, "vari_feature.tiff", sep = ""),  width = 7, heigh = 6)
	return (sc_obj)

}

sr_clst = function(sc_obj, ndim, resdir, proFlag) {
	...
	expr = c("PTPRC", "EPCAM", "ATL1", "CD3D", "CD8A", "CD4", "PDCD1", "TCF7", "NKG7", "MS4A1", "CD27", "CD19", 
	             "CD68", "FCER1A", "CD14", "ITGAX", "IL3RA", "CD63", "NCAM1", "TPSAB1", "GNLY", "FOXP3", "TBX21", "GATA3")
	...
	ggsave(plot = expr_ol, filename = paste(resdir, "pca", ndim ,"expression_overlay_cor.tiff", sep = ""),  width = 16, heigh = 10)

	vln_feat = VlnPlot(sc_obj, features = expr, slot = "counts", log = TRUE, ncol = 3, pt.size = 0.5)
	ggsave(plot = vln_feat, filename = paste(resdir, "pca", ndim, "violin_gene_feature_cor.tiff", sep = ""), width = 12, heigh = 16)
...
sample_type = "vdj_citeseq_expr_112019"#"10X" #"public_br_scRNAseq" #Public scRNA raw data
 #"vdj_citeseq_expr_112019" / "10X"
data_name = "394_results"
...
if (sample_type == "public_br_scRNAseq") {
count_mat <- readRDS(paste(data_dir, "BC3_5_TNBC_all_filter_seurat_project.RDS", sep= ""))
#count_mat <- readRDS(paste(data_dir, "ER_p1246_matrix.RDS", sep= ""))
#count_mat <- readRDS(paste(data_dir, "tumor_matrix.RDS", sep= ""))
#count_mat <- readRDS(paste(data_dir, "all_BC_matrix.RDS", sep= ""))
}
sc_obj <- CreateSeuratObject(counts = count_mat, project = data_name)

if (sample_type == "vdj_citeseq_expr_112019") {
	...
	for (isa in 1:length(inFiles)) {
		...
		#print(tmpObj)
		tmpObj = sr_QC(tmpObj, feaRNA_min = 200, feaRNA_max = 2500, perMT = 10, resdir = res_dir, proFlag = proteinflag)
		srObjs[[isa]] = tmpObj
	}
...
if (batch_cor) {
	integrateList = list("BC377" = bc377_xx_TUMOR,
			     "BC389" = bc389_xx_TUMOR, 
			     "BC392" = bc392_xx_TUMOR,
			     "BC393" = bc393_xx_TUMOR
			     )}
...
sc_obj <- readRDS(paste(data_dir, "all_raw_10X_seurat_objects_list.RDS", sep = ""))
}
if (sample_type == "10X") {
sc_obj <- readRDS(paste(data_dir, "all_filter_10X_seurat_objects_tumor4_list.RDS", sep = ""))
}
...
if (recluster) {
	...
	btc_check <- DimPlot(tc, group.by = "orig.ident")
	ggsave(plot = btc_check, filename = paste(reclst_res_dir, "subset_tcell_b_check.tiff", sep = ""),  width = 9, heigh = 6)
...
if (batch_cor) {
	...
	tc <- FindVariableFeatures(tc, selection.method = "vst", nfeatures = 2000) # nfeatures = (1000,2000)
	top10 <- head(VariableFeatures(tc), 10)
	plot1 <- VariableFeaturePlot(tc)
	plot2 <- LabelPoints(plot = plot1, points = top10, repel = TRUE)
	ggsave(plot = plot2, file = paste(reclst_res_dir, "varfeat_plot.tiff", sep = ""), width = 9, height = 6)
		...
		ggsave(plot = btc, filename = paste(reclst_res_dir, "pca", ndim, "batch_check_aft_cor.tiff", sep = ""), width = 9, heigh = 6)


		expr = c("PTPRC", "CD3E", "CD4", "CD8A", "TCF7", "IL7R", "ITGAE", "PDCD1", "ENTPD1", "DPP4", "FOXP3")
		expr_ol = FeaturePlot(tc_cor, features = expr, reduction = "umap",
						    min.cutoff = "q05", max.cutoff = "q95", ncol = 4, cols = c("orange1", "grey", "purple"))
		ggsave(plot = expr_ol, filename = paste(reclst_res_dir, "pca", ndim, "expression_overlay_cor.tiff", sep = ""),  width = 16, heigh = 10)
		...
		stem_ol = FeaturePlot(tc_cor, features = stem_expr, reduction = "umap",
						    min.cutoff = "q05", max.cutoff = "q95", ncol = 4, cols = c("orange1", "grey", "purple"))
		ggsave(plot = stem_ol, filename = paste(reclst_res_dir, "pca", ndim, "stem_expression_overlay_cor.tiff", sep = ""),  width = 16, heigh = 10)


		vln_feat = VlnPlot(tc_cor, features = expr, slot = "counts", log = TRUE, ncol = 3, pt.size = 0.5)
		ggsave(plot = vln_feat, filename = paste(reclst_res_dir, "pca", ndim, "violin_gene_feature_cor.tiff", sep = ""), width = 12, heigh = 16)
