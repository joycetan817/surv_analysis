### Screen for PD-L1- TAM signature in TCGA-BRCA
### Weihua Guo, Ph.D.
### 11/04/21
...
data_dir <- "C:/Users/wguo/OneDrive - City of Hope National Medical Center/tmp_works/tam_surv/TCGA_BRCA/brca_tcga_pan_can_atlas_2018"
...
res_folder <- paste(hr_subtype, oncotree, surv_type, "pdl1_neg_tam_tcga_brca", sep = "_")
dir.create(file.path(sig_dir, res_folder), showWarnings = FALSE)
ppf <- paste(sig_dir, "/", res_folder, "/", hr_subtype, "_", oncotree, "_", surv_type, "_", sep = "")

cat("Reading TCGA-BRCA data...\n")
expr <- read.table(paste(data_dir, "/data_RNA_Seq_v2_expression_median.txt", sep = ""), header = T, sep = "\t",
...
scln <- read.table(paste(data_dir, "/data_clinical_sample.txt", sep = ""), header = T, sep = "\t",
                   check.names = F, stringsAsFactors = F)
ccln <- as.data.frame(read_excel(paste(data_dir, "/08272019_tcga_pam50_clin.xlsx", sep = "")))
mcln <- merge(pcln, scln, by = "PATIENT_ID", all.y = T)
mcln <- merge(mcln, ccln, by = "PATIENT_ID")
mcln$flag <- TRUE
...
if (surv_type == "DFS") {
  use_cln$RFS_STATUS <- use_cln$DFS_STATUS
  use_cln$RFS_MONTHS <- use_cln$DFS_MONTHS
} else if (surv_type == "DSS") {
  use_cln$RFS_STATUS <- use_cln$DSS_STATUS
  use_cln$RFS_MONTHS <- use_cln$DSS_MONTHS
} else if (surv_type == "PFS") {
  use_cln$RFS_STATUS <- use_cln$PFS_STATUS
  use_cln$RFS_MONTHS <- use_cln$PFS_MONTHS
}
...
ggsave(paste(ppf, 'MCP_multivariant.png', sep = ''), forest_gg, dpi = 300, width = 6, height = 4)
...
write.csv(out_df, paste(ppf, "MCP_use_df_with_cutp.csv", sep = ""))
...
png(paste(ppf, 'MCP_50.png', sep = ''), res = 300, width = 6, height = 4, units = 'in')
print(surv_gg)
gar <- dev.off()
...
surv_gg <- ggsurvplot(fit, data = use_df, pval = TRUE,
                      ...
                      legend.title = 'PD-L1- TAM',
                      legend.labs = c('High', 'Low'))
png(paste(ppf, 'MCP_2575.png', sep = ''), res = 300, width = 6, height = 4, units = 'in')
print(surv_gg)
gar <- dev.off()
...
surv_gg <- ggsurvplot(fit, data = use_df, pval = TRUE,
                      ...
                      legend.title = 'PD-L1- TAM',
                      legend.labs = c('High', 'Low'))
png(paste(ppf, 'MCP_7525.png', sep = ''), res = 300, width = 6, height = 4, units = 'in')
print(surv_gg)
gar <- dev.off()
...
surv_gg <- ggsurvplot(fit, data = tmp_df, pval = TRUE,
                      ...
                      legend.title = 'PD-L1- TAM',
                      legend.labs = c('High', 'Low'))
png(paste(ppf, 'MCP_2525.png', sep = ''), res = 300, width = 6, height = 4, units = 'in')
print(surv_gg)
gar <- dev.off()
...
surv_gg <- ggsurvplot(fit, data = tmp_df, pval = TRUE,
                      ...
                      legend.title = 'PD-L1- TAM',
                      legend.labs = c('High', 'Low'))
png(paste(ppf, 'MCP_3030.png', sep = ''), res = 300, width = 6, height = 4, units = 'in')
print(surv_gg)
gar <- dev.off()
...
fit <- coxph(Surv(RFS_MONTHS, RFS_STATUS) ~ AGE + grade + MCP_sign, data = use_df)
forest_gg <- ggforest(fit)
ggsave(paste(ppf, 'MCP_CD68_multivariant.png', sep = ''), forest_gg, dpi = 300, width = 6, height = 4)
...
surv_gg <- ggsurvplot(fit, data = use_df, pval = TRUE,
                      ...
                      legend.title = 'PD-L1- TAM',
                      legend.labs = c('High', 'Low'))
png(paste(ppf, 'MCP_CD68_50.png', sep = ''), res = 300, width = 6, height = 4, units = 'in')
print(surv_gg)
gar <- dev.off()
...
surv_gg <- ggsurvplot(fit, data = use_df, pval = TRUE,
                      ...
                      legend.title = 'PD-L1- TAM',
                      legend.labs = c('High', 'Low'))
png(paste(ppf, 'MCP_CD68_2575.png', sep = ''), res = 300, width = 6, height = 4, units = 'in')
print(surv_gg)
gar <- dev.off()
...
surv_gg <- ggsurvplot(fit, data = use_df, pval = TRUE,
                      ...
                      legend.title = 'PD-L1- TAM',
                      legend.labs = c('High', 'Low'))
png(paste(ppf, 'MCP_CD68_7525.png', sep = ''), res = 300, width = 6, height = 4, units = 'in')
print(surv_gg)
gar <- dev.off()
...
surv_gg <- ggsurvplot(fit, data = tmp_df, pval = TRUE,
                      ...
                      legend.title = 'PD-L1- TAM',
                      legend.labs = c('High', 'Low'))
png(paste(ppf, 'MCP_CD68_2525.png', sep = ''), res = 300, width = 6, height = 4, units = 'in')
print(surv_gg)
gar <- dev.off()
...
surv_gg <- ggsurvplot(fit, data = tmp_df, pval = TRUE,
                      ...
                      legend.title = 'PD-L1- TAM',
                      legend.labs = c('High', 'Low'))
png(paste(ppf, 'MCP_CD68_3030.png', sep = ''), res = 300, width = 6, height = 4, units = 'in')
print(surv_gg)
gar <- dev.off()
...
use_expr <- use_expr[,intersect(use_cln$SAMPLE_ID, colnames(use_expr))]
use_expr <- log2(use_expr+1)
use_cln <- use_cln[use_cln$SAMPLE_ID %in% colnames(use_expr),]

sig_genes <- rownames(use_expr)
use_df <- as.data.frame(t(use_expr))
use_df$SAMPLE_ID <- rownames(use_df)
use_df <- merge(use_df, use_cln, by = "SAMPLE_ID")
use_df$RFS_STATUS <- ifelse(str_detect(use_df$RFS_STATUS, "0"), 0, 1)
...
write.csv(res, paste(ppf, 'univariant.csv', sep = ''))
...
form_gg <- ggplot(res, aes(x = HR, y = sig)) +
  ...
  theme_classic()
ggsave(paste(ppf, 'univariant.png', sep = ''), form_gg, dpi = 300, width = 6, height = 9)
...
res$sig <- covariates
res$p_format <- str_c('p-val = ', res$p.value)
write.csv(res, paste(ppf, 'univariant_per_cd68.csv', sep = ''))
...
form_gg <- ggplot(res, aes(x = HR, y = sig)) +
  ...
  labs(x = 'Hazard ratio (univariant tests, overall survival)', y = 'Signatures') +
  theme_classic()
ggsave(paste(ppf, 'univariant_per_cd68.png', sep = ''), form_gg, dpi = 300, width = 6, height = 9)
