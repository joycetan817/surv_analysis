...
bar_tiff = paste(res_dir, "barplot.tiff")
tiff(bar_tiff, res=300, width=12, heigh=9, units='in')
plot_pval <- pval_cut[1:length(qcut), "padj"]
if( pval == "cox_reg") {
	pval_cut <- as.data.frame(pval_cut[order(pval_cut$padj),])
	min <- min(pval_cut)
	max <- max(pval_cut)
	barplot(plot_pval, main = paste(gptype, pval, "cutoff"), xlab = "cutoff", ylab = "padj", ylim = c(min,max))
} else {
	barplot(plot_pval, main = paste(gptype, pval, "cutoff"), xlab = "cutoff", ylab = "padj")
}
dev.off()
