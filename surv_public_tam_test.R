...
res_folder = "cutp_TAM_c5_ER+_IDC_ega" # NOTE: Please change this folder name to identify your experiments
...
script_name = "surv_public_tam_test.R"
...
print(max_cutoff)
cutoff_plot <- paste(res_dir,"cutp_geom.tiff", sep="")
ggsave(g, file = cutoff_plot,  dpi = 300, width = 9, height = 6, units = "in", device = "tiff")
...
#                                      surv.median.line = "hv",
		...
		#note_on_plot <- paste("nHigh = ", numh, "\t", "nLow = ", numl, "\t")
		#survcurv$plot <- survcurv$plot + annotate("text", x=2000, y=0.1, label=note_on_plot, size = 6)
surv_plot <- paste(res_dir,"os_survana.tiff", sep="")
ggsave(surv_plot, plot = survcurv$plot, dpi = 300, width = 9, height = 6, units = 'in')
...
#                                      surv.median.line = "hv",
		...
		#note_on_plot <- paste("nHigh = ", numh, "\t", "nLow = ", numl, "\t")
		#survcurv$plot <- survcurv$plot + annotate("text", x=2000, y=0.1, label=note_on_plot, size = 6)
surv_plot <- paste(res_dir,"rfs_survana.tiff", sep="")
ggsave(surv_plot, plot = survcurv$plot, dpi = 300, width = 9, height = 6, units = 'in')
